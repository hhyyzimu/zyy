...
LLIST *llist_creat(int size)
{
      ...
      new->size = size;
      new->head.prev = new->head.next = &new->head;//new->head是一个节点，需要取值
...
//*key的类型：void类型的地址，因为不确定是int char
//*cmp的类型：是一个函数指针，int (*cmp)(const void *,const void *)
{
      struct llist_node_st *node;
...
void llist_travel(LLIST *ptr, llist_op *op)//遍历，用于打印
{
      struct llist_node_st *cur;
