...
{
	char buf_time[32];
	struct passwd *psd;//获取文件所有者用户名
	struct group *grp;//获取文件所有者所属组组名
...
void display_dir(int flag_param, char *path)
{
	...
	//使用冒泡法对文件进行排序，排序后文件名按字母顺序存储于filenames
	for(i = 0; i < count-1; i++)
	{
...
int main(int argc, char **argv)
{
	...
	int flag_param = PARAM_NONE;//参数种类，是否有-l,-a
	struct stat buf;

	/*命令行参数的解析，分析-l,-a,-al,-la*/
	j = 0;
	num = 0;
	for(i = 1; i < argc; i++){
		if(argv[i][0] == '-'){
