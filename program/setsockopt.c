...
/*This function report the error and exits back to the shell*/
...
int main(int argc, char **argv)
{
	...
	if(z)
		bail("setsockopt(a, SOL_SOCKET),""SO_RCVBUF");

	/*As a check on the above...Get socket option SO_SNDBUF:*/
	optlen = sizeof sndbuf;
	z = getsockopt(s, SOL_SOCKET, SO_SNDBUF, &sndbuf, &optlen);
	if(z)
		bail("getsockopt(a, SOL_SOCKET,""SO_SNDBUF)");
