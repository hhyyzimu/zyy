...
int insert(struct node_st **root, struct score *data)//*roor=tree
{
      struct node_st *newnode;

      if (*root == NULL) {
	    newnode = malloc(sizeof(*newnode));//创建第一个结点
	    /* if error */
	    ...
	    *root = newnode;
	    return 0;//先执行这个return，再执行之前也就是下面的return；
      }
...
void balance(struct node_st **root)
{
      ...
      balance(&(*root)->l);//遍历完根平衡之后，遍历根的左使之平衡
      balance(&(*root)->r);
}
